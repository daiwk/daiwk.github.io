...
+ create_dictionaries：
	...
	+ 遍历文件，
	    ...
	    + 每读到空行，表示一个序列结束了，
	        + 进行make_features(sequence)，将这一个序列（比如训练数据有100个序列，这是第33个，而这个序列有三个字符）sequence[33]变成['jetliners', 'NNS', 'I-NP', "'s", '747', 'jetliners', '.', '#E1', '747/jetliners', 'jetliners/.']之类的，长度为feature+3个数的数组（其实就是最开始放进去的那3个元素，再加上每个feature拿到对应的值）
	        + 进行add_to_dict(sequence, dicts)，
