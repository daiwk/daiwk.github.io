...
<!-- TOC -->autoauto- [李宏毅的机器学习笔记](#李宏毅的机器学习笔记)auto- [评价指标](#评价指标)auto    - [关于auc和p/r/f1...](#关于auc和prf1)auto- [auc](#auc)auto- [roc curve v.s. precision-recall curve](#roc-curve-vs-precision-recall-curve)auto    - [ROC与PR曲线](#roc与pr曲线)auto- [过拟合问题](#过拟合问题)auto    - [获取更多数据](#获取更多数据)auto        - [从数据源头获取更多数据](#从数据源头获取更多数据)auto        - [根据当前数据集估计数据分布参数，使用该分布产生更多数据](#根据当前数据集估计数据分布参数使用该分布产生更多数据)auto        - [数据增强（Data Augmentation）](#数据增强data-augmentation)auto    - [使用合适的模型](#使用合适的模型)auto        - [网络结构](#网络结构)auto        - [训练时间（early stopping）](#训练时间early-stopping)auto        - [限制权值/正则化](#限制权值正则化)auto        - [增加噪声](#增加噪声)auto            - [在输入中加噪声](#在输入中加噪声)auto            - [在权值上加噪声](#在权值上加噪声)auto            - [对网络的响应加噪声](#对网络的响应加噪声)auto    - [使用多种模型](#使用多种模型)auto        - [bagging](#bagging)auto        - [boosting](#boosting)auto        - [dropout](#dropout)auto    - [贝叶斯方法](#贝叶斯方法)auto- [模型参数与训练样本规模的关系](#模型参数与训练样本规模的关系)auto    - [How much training data do you need](#how-much-training-data-do-you-need)auto    - [lstm参数规模计算](#lstm参数规模计算)auto    - [embedding layer参数规模计算](#embedding-layer参数规模计算)auto- [其他](#其他)auto    - [不恰当地分割数据](#不恰当地分割数据)auto    - [隐变量](#隐变量)auto    - [定错训练目标](#定错训练目标)auto- [其他2](#其他2)auto    - [cv的tip](#cv的tip)auto- [学习率与batchsize](#学习率与batchsize)autoauto<!-- /TOC -->
...
`\(\sum _{i\in positiveClass} rank_i\)`的含义是所有正样本的rank之和，也就是假设正样本比后面以及自己都大，所以我们要先减掉自己，也就是`\(\sum _{i\in positiveClass} rank_i - M\)`。

然后，对于第一个正样本来讲，后面的M-1个正样本不能和他组pair，同理对于第二个来讲，后面的M-2个不能组pair，所以有`\((M-1)+(M-2)+...+1=M(M-1)/2\)`个要减掉。
