...
- [1. LSTM & GRU](#1-lstm--gru)
    - [1.1 LSTM](#11-lstm)
        - [1.1.1 为何lstm可以缓解梯度消失？](#111-为何lstm可以缓解梯度消失)
    - [1.2 GRU](#12-gru)
- [2. 双向GRU](#2-双向gru)
...
+ 传统的RNN总是用“覆写”的方式计算状态，`\(S_t=f(S_{t-1},x_t)\)`，f是仿射运算后再套sigmoid。所以根据求导的链式法则，梯度会变成连积形式。多个小于1的连乘就会很快接近0
+ 引入门控单元的rnn，使用的是"累加"的方式，`\(S_t=\sum_{\tau=1}^{t}\delta S_{\tau}\)`，
