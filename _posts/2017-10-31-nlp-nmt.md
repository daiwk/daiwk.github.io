...
- [7. 应用](#7-应用)
    - [Backbone模型](#backbone模型)
    - [其他一些尝试](#其他一些尝试)
    - [上述模型存在的问题](#上述模型存在的问题)
    - [采用增强学习进行优化](#采用增强学习进行优化)
    - [reranking](#reranking)
    - [rl进一步改进的方向](#rl进一步改进的方向)
    - [模型融合](#模型融合)
- [8. 其他](#8-其他)
...
其中，
+ `\(\pi _\theta (.)\)`为现有的翻译模型
+ `\tao`为根据现有模型所生成的一个翻译结果
+ `R(\tao)`采用句子级别BLEU进行计算


### reranking

假设模型基本正确，生成k个可能正确的翻译结果，即，采用beamsearch，得到源语言的多个翻译结果，然后根据bleu score计算平均值：
...
然后采用上述结果进行beam search，得到最终结果
8模型融合，相比单模型，+1.1 bleu
