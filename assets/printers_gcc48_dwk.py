...
class Tr1HashtableIterator(Iterator):
    ...
    def __next__ (self):
        ...
        print result, "xxxx"
        #result = node.dereference()['_M_v'].cast(gdb.lookup_type('std::pair<unsigned long const, rec::common::RecNewsInfo*>'))
##        result22 = node.dereference()['_M_v']["second"].dereference().cast(gdb.lookup_type('int')) #dereference()["first"]
        result22 = node.dereference()['_M_v']["second"].dereference().cast(gdb.lookup_type('rec::common::RecNewsInfo'))
        result33 = result22["newsinfo_pb"]["news_category_v1_"] #.dereference() ##.cast(gdb.lookup_type('std::string'))
        ##print self.to_std_string(result33).encoding, "xxxx333"
        print result33
        #print result22, "xxxx222"
        self.node = node.dereference()['_M_nxt']
        return result

    def to_std_string(self, val):
        # Make sure &string works, too.
        type = val.type
        if type.code == gdb.TYPE_CODE_REF:
            type = type.target ()
...
class Tr1UnorderedMapPrinter:
    ...
    def format_one (elt):
        ...
        #return (elt[0]['first'], elt[0]['second'], elt[1])
